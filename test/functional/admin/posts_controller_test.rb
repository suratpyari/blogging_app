...
class Admin::PostsControllerTest < ActionController::TestCase
  ...
  def test_destroy_without_user
    post :destroy, {:id => posts(:post1).id}
    assert_redirected_to '/'
    assert_equal "Login required", flash[:msg]
  end

  def test_destroy_not_a_valid_user
    post :destroy, {:id => posts(:post1).id}, {:user_id => users(:not_admin).id}
  ...
  def test_destroy_valid_user_not_admin
    post :destroy, {:id => posts(:post1).id}, {:user_id => posts(:post1).user_id}
    assert_equal "#{posts(:post1).title} deleted", flash[:msg]
    assert_redirected_to '/'
  end

  def test_destroy_by_admin
    post :destroy, {:id => posts(:post1).id}, {:user_id => users(:admin).id}
    assert_equal "#{posts(:post1).title} deleted", flash[:msg]
    assert_redirected_to '/'
  end
  ...
  def test_update_not_a_valid_user
    post :update,  {:id => posts(:post1).id, :post => {:title => "Changed post title", :content => "Changed post content"}}, {:user_id => users(:not_admin).id}
    assert_equal "You can not edit/ destroy this post as it is not created by you", flash[:msg]
  end

  def test_update_valid_user_not_admin
    post :update,  {:id => posts(:post1).id, :post => {:title => "Changed post title", :content => "Changed post content"}} , {:user_id => posts(:post1).user_id}
    assert_equal "Post updated", flash[:msg]
  end
  
  def test_update_by_admin
    post :update,  {:id => posts(:post1).id, :post => {:title => "Changed post title", :content => "Changed post content"}} , {:user_id => users(:admin).id}
    assert_equal "Post updated", flash[:msg]
  end
